...
import { Physics, useCompoundBody, useCylinder, type CompoundBodyProps, type CylinderProps } from '@react-three/cannon'
    ...
    <Canvas dpr={[1, 2]} camera={{ position: [0, 0, 5], fov: 35 }}>
        <Physics gravity={[0, 1, 0]}>
            {Array.from({ length: amount }, (_, i) => {
                const El = i % 2 ? Geometry : Vercel
                return <El key={i} mass={4} angularDamping={0.4} linearDamping={0.8} position={[Math.random(), Math.random(), Math.random()]} shapes={[]} />
            })}
...
function Geometry(props: CompoundBodyProps) {
    const { nodes } = useGLTF('/assets/geometry.glb')
...
function Cursor({ speed = 10, gradient = 0.7, ...props }) {
    const { nodes } = useGLTF('/assets/cursor.glb')
